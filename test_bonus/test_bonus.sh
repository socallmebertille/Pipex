...
commands=(
    "time ./pipex infile.txt 'sleep 5' 'sleep 3' 'sleep 2' /dev/stdout"
    "./pipex infile.txt 'cat' 'grep And' 'wc -l' /dev/stdout"
    "./pipex infile.txt 'echo 'bonjour'' 'tr 'a-z' 'A-Z'' 'grep B' /dev/stdout"
    "./pipex infile.txt 'cat' 'grep And' 'tr a-z A-Z' 'wc -w' /dev/stdout"
    "./pipex infile.txt 'ls -l' 'grep ^-' 'sort -k 5' 'tail -n 3' /dev/stdout"
    "./pipex infile.txt 'grep -E \"(error|warning)\"' 'sort' 'uniq -c' 'wc -c' /dev/stdout"
    "./pipex infile.txt 'cat' 'tr -s \" \"' 'sort' 'uniq -c' 'sort -nr' /dev/stdout"
    "./pipex infile.txt 'cat' 'grep -oE \"[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+\"' 'sort' 'uniq -c' 'sort -nr' 'head -n 5' /dev/stdout"
)

# Liste des commandes à comparer (vrai pipex)
true_pipe=(
    "time <infile.txt sleep 5 | sleep 3 | sleep 2> /dev/stdout"
